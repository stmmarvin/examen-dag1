...
<main class="flex-1">
    <section class="mx-auto w-full px-8 py-12" style="max-width: 800px;">
        <div class="mb-8 flex items-center justify-between border-b border-[#d7c39a] pb-6">
            <div>
                <h1 class="text-4xl font-bold text-[#0f1f3a]">Medewerker wijzigen</h1>
                <p class="mt-2 text-lg text-[#0f1f3a] opacity-80">Pas de bestaande gegevens van {{ $medewerker->name }} aan</p>
            </div>
        ...
        <div class="rounded-lg border border-[#d7c39a] bg-white p-8 shadow-lg">
            <form method="POST" action="{{ route('medewerkers.update', $medewerker) }}">
                ...
                <div class="mb-6 rounded-md border border-[#d7c39a] bg-[#f8f4ea] px-4 py-3 text-sm font-semibold text-[#0f1f3a]">
                    De velden hieronder bevatten de huidige gegevens. Wijzig wat nodig is en klik daarna op opslaan.
                </div>
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        <div>
                            ...
                            <input id="voornaam" type="text" name="voornaam" value="{{ old('voornaam', $medewerker->voornaam) }}" required
                                   class="w-full rounded-md border-2 border-[#d7c39a] bg-white px-4 py-3 text-[#0f1f3a] focus:border-[#c69a3e] focus:outline-none">
                            @error('voornaam')
                        ...
                        <div>
                            ...
                            <input id="achternaam" type="text" name="achternaam" value="{{ old('achternaam', $medewerker->achternaam) }}" required
                                   class="w-full rounded-md border-2 border-[#d7c39a] bg-white px-4 py-3 text-[#0f1f3a] focus:border-[#c69a3e] focus:outline-none">
                            @error('achternaam')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email', $medewerker->email) }}" required
                               class="w-full rounded-md border-2 border-[#d7c39a] bg-white px-4 py-3 text-[#0f1f3a] focus:border-[#c69a3e] focus:outline-none">
                        @error('email')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input id="telefoon" type="tel" name="telefoon" value="{{ old('telefoon', $medewerker->telefoon) }}"
                    ...
                    <div class="border-t border-[#d7c39a] pt-6">
                        <label for="password" class="mb-2 block text-sm font-semibold text-[#0f1f3a]">
                            Nieuw wachtwoord
                        </label>
                        <input id="password" type="password" name="password"
                               class="w-full rounded-md border-2 border-[#d7c39a] bg-white px-4 py-3 text-[#0f1f3a] focus:border-[#c69a3e] focus:outline-none">
                        <p class="mt-2 text-sm text-[#0f1f3a] opacity-70">Laat leeg als het wachtwoord hetzelfde moet blijven.</p>
                        @error('password')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
