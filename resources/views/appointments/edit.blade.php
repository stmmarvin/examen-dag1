<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <p class="text-gray-600 mb-6">Wijzig de gegevens van de afspraak.</p>
                        ...
                        <form method="POST" action="{{ route('appointments.update', $appointment) }}">
                                    ...
                                    <select name="appointment_time" id="appointment_time" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                        ...
                                        @for($hour = 9; $hour < 18; $hour++)
                                            @foreach(['00', '15', '30', '45'] as $minute)
                                                ...
                                                @endphp
                                                <option value="{{ $time }}" {{ old('appointment_time', $appointment->appointment_date->format('H:i')) == $time ? 'selected' : '' }}>
                                                    {{ $time }}
                                                </option>
                                            @endforeach
                                        @endfor
                            ...
                            <div class="mb-6">
                                ...
                                <select name="status" id="status" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    <option value="in afwachting" {{ old('status', $appointment->status) == 'in afwachting' ? 'selected' : '' }}>In afwachting</option>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea name="notes" id="notes" rows="4" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Voeg een notitie toe...">{{ old('notes', $appointment->notes) }}</textarea>
                                @error('notes')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="bg-gray-50 shadow-sm rounded-lg p-6 sticky top-6">
                        ...
                        <div class="space-y-3 text-sm">
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                ...
                                <span class="font-medium" id="summary-client">{{ $appointment->client->full_name }}</span>
                            </div>
                            
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="text-gray-600">Medewerker</span>
                                <span class="font-medium" id="summary-employee">{{ $appointment->employee->name }}</span>
                            </div>
                            
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="text-gray-600">Behandeling</span>
                                <span class="font-medium" id="summary-treatment">{{ $appointment->treatment->name }}</span>
                            </div>
                            
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="text-gray-600">Datum & tijd</span>
                                <span class="font-medium" id="summary-datetime">{{ $appointment->appointment_date->format('d M Y H:i') }}</span>
                            </div>
                            
                            <div class="flex justify-between py-2 border-b border-gray-200">
                                <span class="text-gray-600">Duur</span>
                                <span class="font-medium" id="summary-duration">{{ $appointment->duration_minutes }} minuten</span>
                            </div>
                            
                            <div class="flex justify-between py-2 font-semibold">
                                <span class="text-gray-900">Prijs</span>
                                <span class="text-gray-900" id="summary-price">€{{ number_format($appointment->treatment->price, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
